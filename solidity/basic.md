...
이더리움은 SHA3의 버전인 keccak256를 내장 해시 함수로 가지고 있다.

해시함수는 기본적으로 입력 스트링을 랜덤 256비트 16진수로 매핑한다. 스트링에 약간의 변화라도 있으면 해시값은 크게 달라짐.

해시 함수는 이더리움에서 여러 용도로 활용되지만, 여기서는 의사 난수 발생기로(pseudo-random number generator)로 이용 할것임.
...
>참고 : 블록체인에서 안전한 의사 난수 발생기는 매우 어려운 문제.
    여기서 우리가 활용하는 방법은 안전하지 않지만, 좀비 DNA에 있어서 보안은 최우선 순위가 아니니 우리의 목적엔 적합함.
...
이벤트는 contract가 블록체인상에서 앱의 사용자 단에서 무언가 액션이 발생했을 때 의사소통하는 방법

contract는 특정이벤트가 일어나는지 귀를 기울이고 그 이벤트가 발생하면 행동을 취함.
...
사용자단의 해당 이벤트가 일어나는지 자바스크립트로 이를 구현하면 다음과 같다.
```javascript
...
Contract와 상호작용하는 사용자단의 자바스크립트 코드를 작성해야함.
